...
def get_candidates(hashed_A, hashed_point, query_index):
    '''hashed_point 中含有L个hash字符串，hashed_A为高维数组 hashed_A[i]中只要有一串
    hash字符串与hashed_point中的字符串相同 就是candiadates points 即 any(hashed_point == hashed_A[i])'''
...
def linear_search(A, query_index, num_neighbors):
    ...
    for row_index in range(len(A)):
        if row_index != query_index:
            ...
            if len(neighbors_arr) < num_neighbors:
                # neighbors_arr中只容纳num_neighbors个point，如果数组中的元素个数小于阈值 直接加入新元素
                ...
                # 如果个数大于阈值 首先检查新加入元素distance是否大于现数组中的最大值
                if distance > neighbors_arr[-1][1]:
                    continue
                # 如果小于最大值，则倒叙遍历数组 寻找新元素的插入位置 舍弃该位置的旧元素 插入新元素
                candidate_idx = -1
    ...
    return [neighbor[0] for neighbor in sorted_arr] # 返回排序数组
